...
if (isset($_GET['nama'])){
	$cari=$_GET['nama'];
	$query=mysql_query("SELECT * FROM mahasiswa WHERE nama LIKE '%$cari%' LIMIT $posisi,$itemperpage");
	$jumlahdata=@mysql_num_rows(mysql_query("SELECT * FROM mahasiswa WHERE nama LIKE '%$cari%'"));
	$link="nama=".$cari."&";}
else {
	$query=mysql_query("SELECT * FROM mahasiswa ORDER BY angkatan asc, semester desc, nama asc LIMIT $posisi,$itemperpage");
	$jumlahdata=@mysql_num_rows(mysql_query("SELECT * FROM mahasiswa"));}
...
<html>
	...
	<div class="content">
		...
		<div class="isi">
			<h1 class="judul">Data Mahasiswa 
			<?php if ($level==admin){ ?><a href="input-mhs.php">
    ...
    <thead>
		...
		<tbody>	
			<?php while ($baris=mysql_fetch_array($query)){
				...
				if ($level=='admin'){echo "<td> <a href='edit-mhs.php?idm=".$baris[0]."'>".$baris[3]."</a></td>";}
				else {echo "<td>".$baris[3]."</td>";}
				echo "<td>".$baris[4]."</td>";
				echo "<td>".$baris[5]."</td>";
				echo "<td>".$baris[6]."</td>";
				echo "<td>".$baris[7]."</td>";
...
//=====================================================================halaman				
				...
				if ($hal>3){
					echo "<a href='".$_SERVER['HTTP_SERVER']."/kinerja/admin/mahasiswa.php?".$link."'>first</a>";}
					
				if ($jumlahhalaman>1){
					for ($i=$blockhal; $i<=$itemblok; $i++){
						echo "<a class='paging' href='".$_SERVER['HTTP_SERVER'].
						"/kinerja/admin/mahasiswa.php?".$link."h=".$i."'>".$i."</a>";}
					}
						
				if ($jumlahhalaman>5 && $jumlahhalaman-2 > $hal){
					echo "<a href='".$_SERVER['HTTP_SERVER'].
					"/kinerja/admin/mahasiswa.php?".$link."h=".$jumlahhalaman."'>last</a>";}
